...
The expected number of times a sequence of 7 head will pop up after 1000 flips (rounded to the nearest whole number) is `r round(mean(runs_of_6))`. Try different lengths to see the expected value of different sequence lengths.
...
**Conclusion:** The 5% annual VaR of the portfolio is `r as.integer(VaR)`, in other words, there is a 0.05 probability that the portfolio will fall in value by more than `r as.integer(VaR)` over a one year period if there is no trading.
